...
All relics and trinkets have individual collectible art in rewards, shops, the combat bar, and the loadout view. The three playable leads have separate portraits; each act and boss has its own backdrop. Combat uses labeled status badges, Flow markers, and distinct particle cues for strikes, enemy hits, shields, healing, card draws, planning, status effects, and teamwork. Combat effects are disabled for reduced-motion settings. The collectible, role, and backdrop prompts are in `public/assets/collectibles-and-scenes-prompts.md`.
