...
Five more skills turn existing systems into sharper choices. **Critical Path Control** advances a chosen initiative or gate, while **Kill Switch** abandons one for immediate capacity and Debt. **Precision Counter** can spend Evidence to stop a foe's telegraphed non-attack action, or simply guard; the new **Red Tape Warden** delays a running initiative or taxes next turn's SP. **Hotfix Forge** temporarily rewrites a basic card or Open Defect into an exhausting Prototype, without removing the original from the run deck. **Emergency Contractor** trades credits for immediate SP and a card, then charges an onboarding SP penalty in the next fight. Each card has a tailored Force/Flex upgrade. The four generated card scenes, Warden portrait, and exact prompts are in `public/assets/cards/meaningful-skills-prompts.md`.
...
| Left / Right | Page through a Salvage, Reclaim, or project choice |
